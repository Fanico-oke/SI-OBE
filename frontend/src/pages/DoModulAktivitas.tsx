...
export const DoModulAktivitas = () => {
  ...
  const [loadingAsesmen, setLoadingAsesmen] = useState(false);
  ...
  // Fetch kelas detail + asesmen when selected
  useEffect(() => {
    ...
    const fetchAsesmen = async () => {
      try {
        setLoadingAsesmen(true);
        const res = await axios.get(`/api/do/kelas/${selectedKelasId}/asesmen`);
        const data = Array.isArray(res.data) ? res.data : res.data.data || [];
        setAsesmenList(data);
      } catch {
        setAsesmenList([]);
    ...
    <div className="w-full space-y-4">
      <div className=" w-full">
        <div className="flex gap-6 border-b border-outline-variant overflow-x-auto">
          ...
          <Link to="/do/aktivitas" className="tab-active pb-3 whitespace-nowrap px-2">Aktivitas</Link>
...
{/*  Section 3: Tugas & Asesmen / Aktivitas  */}
{activeTab === 'tugas' && (
<div className="bg-surface rounded-lg border border-outline-variant p-6">
<div className="flex justify-between items-center mb-4">
<h4 className="font-h3 text-h3 text-on-background flex items-center gap-2">
...
{asesmenList.map((asesmen, idx) => (
  <div key={asesmen.id || idx} className="p-4 border border-outline-variant rounded-lg bg-surface-container-lowest flex flex-col sm:flex-row gap-4 items-start sm:items-center justify-between">
    <div className="flex items-start gap-3">
      <div className="mt-1">
        <span className="material-symbols-outlined text-secondary" >task</span>
      </div>
      <div>
        <p className="font-body text-body font-medium text-on-background">{asesmen.nama}</p>
        <p className="font-caption text-caption text-on-surface-variant mt-1 flex items-center gap-2">
          {asesmen.bobot !== undefined && (
            <span className="px-2 py-0.5 bg-surface-container rounded text-on-surface font-data-mono">Bobot: {asesmen.bobot}%</span>
          )}
          {asesmen.jenis && (
            <span className="px-2 py-0.5 bg-secondary/10 rounded text-secondary font-data-mono">{asesmen.jenis}</span>
          )}
        </p>
      </div>
    </div>
    ...
    <div className="flex items-center gap-2 self-end sm:self-auto">
      <button onClick={() => addToast('Edit asesmen di menu Do > Penilaian', 'info')} className="btn-secondary px-3 py-1.5 rounded text-caption">Edit</button>
    </div>
    )}
  </div>
))}
